<form id="bank-transfer">
	...
	<fieldset>
		...
		<div class="content">
			...
			<?php
			if ( empty( $bt_message ) && ! $bt_message = $HMP_options->gateways['bank-transfer']['message'] ) {
	...
	<fieldset>
		...
		<div class="content">
			...
			<p><strong><?php _e( 'Order Total:', APP_TD ); ?></strong> <?php echo hmp-theme_get_price( $order->get_total(), $order->get_currency() ); ?></p>
	...
	<?php if ( hmp-theme_get_checkout() ) { ?>
	<fieldset>
		<input type="submit" class="button" value="<?php _e( 'Continue &rsaquo;&rsaquo;', APP_TD ); ?>"  onClick="location.href='<?php echo esc_attr( add_query_arg( array( 'bt_end' => 1 ), hmp-theme_get_step_url( 'order-summary' ) ) ); ?>';return false;">
	</fieldset>
	<?php } ?>
