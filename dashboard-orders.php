<?php if ( ( $orders = HMP_get_user_dashboard_orders() ) || get_query_var('order_status') ) : ?>

	<p><?php _e( 'Below is your Order history. You can use the provided filter to filter all the orders.', APP_TD); ?></p>

	<form class="filter" method="get" action="<?php echo esc_url( HMP_DASHBOARD_ORDERS_URL ) ?>" >
		<input type="hidden" name="tab" value="orders" />
		<?php foreach( HMP_get_order_statuses_verbiages() as $order_status => $name ): ?>
		...
		<?php if ( get_query_var('order_status') ): ?>
			&mdash; <a href="<?php echo esc_url( add_query_arg( 'tab', 'orders', HMP_DASHBOARD_ORDERS_URL ) ); ?>"><?php _e( 'Remove Filters', APP_TD ); ?></a>
		<?php endif; ?>
	...
	<?php else: ?>
		...
		<table cellpadding="0" cellspacing="0" class="tblwide footable tablet footable-loaded">
			...
			<?php if ( $orders->have_posts() ) : ?>
				...
				<?php while ( $orders->have_posts() ) : $orders->the_post(); ?>
					...
					<?php $order = hmp-theme_get_order( $orders->post->ID ); ?>
						<tr>
							...
							<td class="order-history-summary left">
								...
								<?php echo HMP_get_the_order_summary( $order, $output_type = 'html' ); ?>
							</td>
							<td class="order-history-price center"><?php echo hmp-theme_get_price( $order->get_total() ); ?></td>
		...
		<?php hmp-theme_pagination( '', '', $orders ); ?>
