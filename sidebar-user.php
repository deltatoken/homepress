...
global $current_user, $HMP_options;

$logout_url = HMP_logout_url();
?>
...
<div class="content_right">
	...
	<div class="shadowblock_out">
		...
		<div class="shadowblock">
			...
			<?php if ( has_nav_menu( 'theme_dashboard' ) ) : wp_nav_menu( array( 'theme_location' => 'theme_dashboard', 'menu_id' => 'menu-dashboard', 'container' => false ) ); else : ?>
				...
				<ul>
					<li><a href="<?php echo HMP_DASHBOARD_URL; ?>"><?php _e( 'My Ads', APP_TD ); ?></a></li>
					<li><a href="<?php echo HMP_DASHBOARD_ORDERS_URL; ?>"><?php _e( 'My Orders', APP_TD ); ?></a></li>
					<li><a href="<?php echo HMP_PROFILE_URL; ?>"><?php _e( 'Edit Profile', APP_TD ); ?></a></li>
	...
	<div class="shadowblock_out">
		...
		<div class="shadowblock account-info">
			...
			<div class="avatar"><?php hmp-theme_get_profile_pic( $current_user->ID, $current_user->user_email, 140 ); ?></div>

				<ul class="user-info">
					<li><h3 class="single"><a href="<?php echo get_author_posts_url( $current_user->ID ); ?>"><?php echo $current_user->display_name; ?></a></h3></li>
					<li><strong><?php _e( 'Member Since:', APP_TD ); ?></strong> <?php echo hmp-theme_display_date( $current_user->user_registered, 'datetime', true ); ?></li>
					<li><strong><?php _e( 'Last Login:', APP_TD ); ?></strong> <?php echo hmp-theme_get_last_login( $current_user->ID ); ?></li>
				</ul>

				<?php if ( $HMP_options->enable_membership_packs ) { ?>
				<?php $membership = HMP_get_membership_package( $current_user->active_membership_pack ); ?>
				<ul class="membership-pack">
					<?php if ( $membership && ( hmp-theme_days_between_dates( $current_user->membership_expires ) < 0 ) ) { ?>
						<li><?php printf( __( 'Your membership pack "%1$s" expired on %2$s.', APP_TD ), $membership->pack_name, hmp-theme_display_date( $current_user->membership_expires ) ); ?></li>
						<li><a href="<?php echo HMP_MEMBERSHIP_PURCHASE_URL; ?>"><?php _e( 'Renew Your Membership Pack', APP_TD ); ?></a></li>
					<?php } elseif( $membership ) { ?>
						<li><strong><?php _e( 'Membership Pack:', APP_TD ); ?></strong> <?php echo $membership->pack_name; ?></li>
						<li><strong><?php _e( 'Membership Expires:', APP_TD ); ?></strong> <?php echo hmp-theme_display_date( $current_user->membership_expires ); ?></li>
						<li><a href="<?php echo HMP_MEMBERSHIP_PURCHASE_URL; ?>"><?php _e( 'Renew or Extend Your Membership Pack', APP_TD ); ?></a></li>
					<?php } else { ?>
						<li><a href="<?php echo HMP_MEMBERSHIP_PURCHASE_URL; ?>"><?php _e( 'Purchase a Membership Pack', APP_TD ); ?></a></li>
					<?php } ?>
				</ul>
				<?php } ?>
				...
				<ul class="user-details">
					...
					<?php if ( ! empty( $current_user->facebook_id ) ) { ?><li><div class="dashicons-before facebookico"></div><a href="<?php echo hmp-theme_make_fb_profile_url( $current_user->facebook_id ); ?>" target="_blank"><?php _e( 'Facebook', APP_TD ); ?></a></li><?php } ?>
				...
				<?php HMP_author_info( 'sidebar-user' ); ?>
...
?>
	...
	<?php hmp-theme_before_sidebar_widgets( 'user' ); ?>
	...
	<?php hmp-theme_after_sidebar_widgets( 'user' ); ?>
